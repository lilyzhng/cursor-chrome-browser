// Background service worker for the Cursor Chrome Browser extension.
// Handles: WebSocket link to the MCP server, CDP via chrome.debugger, tool dispatch,
// tab group management.
//
// Tool logic (tab groups, CDP helpers, the toolHandlers map) is borrowed verbatim from
// the open-claude-in-chrome reference (MIT). The transport is rewritten: instead of Chrome
// native messaging + a native host process, the extension connects directly to the MCP
// server over a localhost WebSocket. See docs/design.md.
...
const SERVER_URL = `ws://127.0.0.1:${DEFAULT_PORT}`;
...
// --- Keep-alive alarm: also reconnects the WebSocket if it dropped ---
...
// --- WebSocket link to the MCP server ---
function connectServer() {
  ...
  try {
    ...
    ws.addEventListener("close", () => {
      ws = null;
      // Retry in 2 seconds (server may not be up yet)
...
function sendResponse(id, result) {
  ...
  try {
    ws.send(JSON.stringify({ id, type: "tool_response", result }));
  } catch {
    // Socket closed
  }
}
...
function sendError(id, error) {
  if (!ws || ws.readyState !== WebSocket.OPEN) return;
  try {
    ws.send(JSON.stringify({ id, type: "tool_error", error: String(error) }));
  } catch {
    // Socket closed
  }
}
...
const toolHandlers = {
  ...
  async read_page(args) {
    ...
    // Append viewport dimensions so the model knows the coordinate space
    try {
      await ensureAttached(tabId);
      const vp = await cdp(tabId, "Runtime.evaluate", {
        expression: "window.innerWidth + 'x' + window.innerHeight",
  ...
  async upload_image(args) {
    ...
    if (ref) {
      ...
      const result = await cdp(tabId, "Runtime.evaluate", {
        expression: `(() => {
          const el = window.__cursorChromeBrowser?.resolveRef?.("${ref}");
          if (!el) return null;
  ...
  async switch_browser(args) {
    return { content: [{ type: "text", text: "Browser switching is not yet supported. The extension connects to whichever browser has it loaded. To switch, disable the extension in the current browser, enable it in the target browser, and restart both." }] };
  },
